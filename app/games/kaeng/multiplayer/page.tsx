...
export default function KaengMultiplayer() {
  ...
  const startGame = async () => {
    ...
    const dealt = room.players.map(p => {
      const rawHand = newDeck.splice(0, 5)
  ...
  const actionDraw = async () => {
    ...
    const rawHand = [...(me?.hand??[]), drawn]
    const newHand = sortHand(rawHand)   // เรียงไพ่ใหม่
    const logMsg = canFlow
      ? `⚠️ โง่! ${me?.username} จั่วทั้งที่ไหลได้ · จั่ว ${drawn.value}${drawn.suit}`
      : `${me?.username} จั่ว ${drawn.value}${drawn.suit}`
    const newLog = await addLog(logMsg, room.gameLog)
  ...
  const actionFlow = async () => {
    ...
    // ตรวจตบ: มีไพ่ค่าเดียวกัน ≥ 2 ใบ
    const slapCards = myHand.filter(c => c.value === top.value)
    ...
    const newHand = sortHand(rawNewHand)
    const newDp = isSlapping
      ? [...room.discardPile, ...slapCards]
  ...
  const actionDiscard = async () => {
    ...
    const newPlayers = room.players.map((p,i)=>{
      if (i===myIdx) return {...p, hand:sortedHand, score:handScore(sortedHand), action:'WAITING' as PlayerAction}
            ...
            <div className="flex items-center justify-center gap-8">
              ...
              {slapNotice && (
                <div className={`text-4xl font-black animate-bounce text-center
                  ${slapNotice === 'โง่!' ? 'text-red-400' : 'text-yellow-300'}`}>
                  {slapNotice}
                </div>
              )}
          )}
            ...
            {room.phase==='PLAYING' && isMyTurn && !mustDiscard && (
              <div className="grid grid-cols-3 gap-3">
                ...
                <button onClick={actionFlow} disabled={!canFlow}
                  ...
                  {canFlow && isSlapPossible ? 'ตบ! 🎉' : canFlow ? 'ไหล ✓' : 'ไหล'}
                </button>
